    ...
    return Scaffold(
      body: LayoutBuilder(
        builder: (context, constraints) {
          ...
          if (isMobile) {
            if (isEditorVisible) {
              return BannerEditorForm();
            } else {
              return const _BannersTable();
            }
          }

          return Row(
            children: [
              Expanded(flex: isEditorVisible ? 3 : 5, child: const _BannersTable()),
              if (isEditorVisible) ...[
...
class _BannersTable extends StatefulWidget {
  const _BannersTable();

  @override
  State<_BannersTable> createState() => _BannersTableState();
}

class _BannersTableState extends State<_BannersTable> {
